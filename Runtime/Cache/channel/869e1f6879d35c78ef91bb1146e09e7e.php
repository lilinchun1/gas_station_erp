    ...
    <div class="role-inquire channel-index-btns">
        <form name="channelSelect" method="get" action="<?php echo U('channel/Channel/channelSelect');?>">
            <p>
				...
				value="<?php echo ($_GET['channel_name_txt']); ?>" onfocus="blurry('channel_name','<?php echo U('channel/Channel/getAllLike');?>',this)"/>
                ...
                value="<?php echo ($_GET['agent_name_txt']); ?>" onfocus="blurry('agent_name','<?php echo U('channel/Channel/getAllLike');?>',this)"/>
            ...
            <p>
                ...
                <input type="submit" class="role-control-btn" value="查询" /><input type="button" id="channelDala" class="role-control-btn" value="清空"/> 
            </p>
        </form>
    </div>
    ...
    <div class="bd over-h-y">
        <ul class="role-table-list">
            ...
            <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><li class="list_sel" onClick="selectChannelRadio('<?php echo ($vo['channel_id']); ?>','<?php echo ($vo['channel_type_father_id']); ?>',
                    ...
                    <span class="span-1">
						<input type="radio" name="channelDetailID" value="<?php echo ($vo['channel_id']); ?>" 
		...
		$("#b_add_channel").click(function () {
			...
			$.post(getAgentUrl,{},function(data){
				...
				$.each(data,function(i,n){
					$("#add_agent_id_sel").append("<option value='"+n['agent_id']+"'>"+n['agent_name']+"</option>");
				})
			},"json");
             $.openDOMWindow({
			     loader:1,
...
//========================================================
			...
			$.getJSON(handleUrl,{"add_channel_name_txt":add_channel_name_txt,"add_agent_id_sel":add_agent_id_sel,
								 ...
								 "add_channel_address_txt":add_channel_address_txt
								 },
				function (data){
		...
		$("#b_change_channel").click(function () {
			...
			$.getJSON(handleUrl,{"channel_id":channel_val},
				function (data){
					...
					var getAgentUrl = "<?php echo U('configuration/Org/show_org_tree');?>";
					$.post(getAgentUrl,{},function(data){
						$("#change_belong_agent_id_sel option").remove();
						$("#change_belong_agent_id_sel").append("<option value=''>请选择组织</option>");
						$.each(data,function(i,n){
							if( n['agent_id'] == belong_agent_id)
							{
								$("#change_belong_agent_id_sel").append("<option value='"+n['agent_id']+"' selected='selected' >"+n['agent_name']+"</option>");
							}
							else
							{
								$("#change_belong_agent_id_sel").append("<option value='"+n['agent_id']+"'>"+n['agent_name']+"</option>");
							}
						})
					},"json");
					...
					//alert(data['channel_address']);return;
		...
		$('#submit_change_channel').click(function(){
			...
			$.getJSON(handleUrl,{"change_channel_name_txt":change_channel_name_txt,"change_channel_id_txt":change_channel_id_txt,
								"change_agent_id_sel":change_agent_id_sel,"change_dst_province":change_dst_province,"change_dst_city":change_dst_city,
								 ...
								 "change_channel_address_txt":change_channel_address_txt,
								 ...
								 //"change_channel_tel_txt":change_channel_tel_txt,
								 },
				function (data){
					//alert(data);return;
